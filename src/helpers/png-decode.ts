...
export function decodePngRgba(path: string): { data: Buffer; width: number; height: number } {
  ...
  if (bitDepth !== 8) throw new Error(`decodePngRgba: unsupported bit depth ${bitDepth}`)

  const channels = colorType === 6 ? 4 : colorType === 2 ? 3 : colorType === 0 ? 1 : 0
  if (channels === 0) throw new Error(`decodePngRgba: unsupported colour type ${colorType}`)
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < stride; x++) {
      ...
      let value = line[x]
      switch (filter) {
        case 1: value = (value + left) & 0xff; break // Sub
        case 2: value = (value + up) & 0xff; break // Up
        case 3: value = (value + ((left + up) >> 1)) & 0xff; break // Average
        case 4: { // Paeth
