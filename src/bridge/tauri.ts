 ...
 */
export const enum BridgeDownstreamEvent {
...
export class BridgeClient {
  ...
  /** 当前重连延迟（指数退避）。 */
  private reconnectDelay = SSE_INITIAL_RECONNECT_DELAY_MS
  /** 重连定时器 id。 */
   ...
   */
  connect(): void {
    if (this.disposed || this.eventSource !== null) return
   ...
   */
  async post(endpoint: BridgeUpstreamEndpoint | string, body: BridgePostBody): Promise<unknown | null> {
   ...
   */
  async reportWorkspacePath(path: string | null): Promise<void> {
   ...
   */
  async reportSessionFocus(sessionId: string | null): Promise<void> {
   ...
   */
  async reportThemeChange(dark: boolean): Promise<void> {
 ...
 */
export function connectBridge(baseUrl?: string): BridgeClient | null {
...
export async function postBridge(
  endpoint: BridgeUpstreamEndpoint | string,
  body: BridgePostBody,
  baseUrl?: string,
