          ...
          return readJsonBody(req).then(
            (body) => {
              const record = (typeof body === 'object' && body !== null)
                ...
                : {}
              // Narrow to known fields only.
              const patch: Partial<ShellConfig> = {}
              if (record.windowOpen === 'auto' || record.windowOpen === 'manual') patch.windowOpen = record.windowOpen
              if (typeof record.width === 'number' && record.width >= 480) patch.width = Math.floor(record.width)
              if (typeof record.height === 'number' && record.height >= 360) patch.height = Math.floor(record.height)
