...
function sanitize(raw: unknown, base: TerminalPrefsDoc = {}): TerminalPrefsDoc {
  const input = (raw ?? {}) as Record<string, unknown>
  const out: TerminalPrefsDoc = { ...base }
  ...
  if (typeof input.shell === 'string' && SHELL_IDS.includes(input.shell)) out.shell = input.shell
  return out
}

/** Read the persisted prefs; missing/corrupt file yields an empty doc. */
export function readTerminalPrefs(): TerminalPrefsDoc {
  try {
 ...
 */
export function writeTerminalPrefs(patch: unknown): TerminalPrefsDoc | null {
