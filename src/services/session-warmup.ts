...
/** Structural view of the two optional services the warm-up touches. */
...
async function inspectOne(ctx: Context, persistence: SessionPersistenceLike, id: string): Promise<boolean> {
 ...
 */
export function installSessionWarmup(ctx: Context): () => void {
    ...
    void (async () => {
      try {
        ...
        for (const h of headers.slice(0, WARMUP_COUNT)) {
          ...
          if (await inspectOne(ctx, persistence, h.id)) ok++
        }
  ...
  try {
    const off = ctx.on('session/disposed', (session: { id?: string }) => {
      const id = session?.id
      if (typeof id !== 'string' || id === '') return
      const persistence = ctx.get('sessionPersistence') as SessionPersistenceLike | undefined
      if (persistence?.inspect === undefined) return
