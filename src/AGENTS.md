...
src/
  ...
  services/         Services：纯领域逻辑（config-store / pty-manager）
  ...
  plugins/          独立插件（@dsh-external/*，双轨分发见根 AGENTS §1.1 铁律 8；必须接装配链 cordis.patch.yml+profile，禁死目录；4 插件均已 private:false + publishConfig，npm 轨发布前必须过插件轻量门禁 + pack 清单核对）
```
...
| `services/pty-manager.ts` | **Services** | 交互终端 PTY 会话管理（M4，node-pty）：createPty / ptyWrite / ptyResize / ptyClose / ptySubscribe / disposeAll；危险命令 UX 护栏（BLOCKED_CMDS，非安全边界）；taskkill /T /F 进程树清理 |
| `server/pins-api.ts` | **Server（薄路由）** | 置顶会话持久化（pins.json 原子写，readPinnedSessions / writePinnedSessions 内联于此，无独立 services/pins-store.ts） |
...
- **禁止**删除/清空 `C:\Users\*`、`~`、`$HOME`、真实 `~/.dsh` 下任何文件；删除目标路径不得含 `~`、`$HOME`、`USERPROFILE`、`C:\Users`、`homedir()`。
- 测试/复现/验证一律用**隔离 DSH_HOME**（E:/D:/`%TEMP%` 独立目录），**永不**删真实 `~/.dsh`（配置/会话/凭据不可恢复）。
- 执行删除前先打印目标绝对路径并确认不是 home 子树；破坏性操作不得在 `never` 审批模式下静默执行。
...
4. **退出语义**：进程退出归 **Rust 壳**（`src-tauri/src/helpers/quit.rs`：写 `quit.marker` + `process::exit(0)`；supervisor 据此区分主动退出/崩溃）。host 侧**不**写 marker、**不** `process.exit()`（WebView2 时代 `exitProcess()` 已删除，勿复活）；**禁止** `ctx.appExit` / `app.exit()`。
...
- **单向依赖**：上层可依赖下层，**下层禁止依赖上层**。层级：`index.ts → controllers → core → managers/server/services → helpers → models/utils`。
  - `server/*-api.ts` 只调 `helpers/*`、`../services/*`（同层服务，如 config-store / pty-manager）与纯库，不 import `index.ts` / `managers` 业务。
