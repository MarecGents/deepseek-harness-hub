...
export function installConversationRail(ctx: unknown): () => void {
  ...
  function findScrollContainer(from: HTMLElement): HTMLElement | null {
    ...
    // prefer it (document-wide) over the generic walk.
    const explicit = document.querySelector<HTMLElement>('[data-conversation-scroll]')
    if (explicit !== null) return explicit
    // Walk the slot upward to its ancestors, then its subtree — the scroll
    // viewport may be an ancestor rather than a descendant.
    const stack: HTMLElement[] = []
    let cursor: HTMLElement | null = from
    while (cursor !== null) {
      stack.push(cursor)
      cursor = cursor.parentElement
    }
    stack.push(...Array.from(from.querySelectorAll<HTMLElement>('*')))
    // Prefer a viewport that is actually scrollable (content overflows).
    ...
    // Fallback: an explicit scroll container even when content is short (it
    // still scrolls as messages grow).
    for (const el of stack) {
      const overflowY = getComputedStyle(el).overflowY
  ...
  function extractTurnSummaries(snapshot: ConversationSnapshotLike | undefined): string[] {
    ...
    for (let t = 0; t < count; t += 1) {
      ...
      // Prefer the user's own message as the preview ("我这边"); fall back to
      // the reply only when this turn had no human message.
  ...
  function showTip(_tick: HTMLElement, index: number, text: string): void {
    const el = ensureTip()
  ...
  function renderTicks(): void {
    ...
    // Ticks tightly packed with a small fixed gap, vertically centered.
