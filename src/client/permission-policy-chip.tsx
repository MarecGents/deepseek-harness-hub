...
export const PERMISSION_POLICIES: readonly PermissionPolicy[] = ['follow', 'strict', 'read-only']
...
export function permissionPolicyLabel(policy: PermissionPolicy | null): string {
  switch (policy) {
...
export async function fetchPolicy(): Promise<PermissionPolicy | null> {
  try {
    const res = await fetch(POLICY_ROUTE)
    if (!res.ok) return null
    const body = (await res.json()) as { ok?: boolean; policy?: PermissionPolicy }
...
export async function savePolicy(policy: PermissionPolicy): Promise<boolean> {
  try {
    const res = await fetch(POLICY_ROUTE, {
      method: 'POST',
...
export function PermissionPolicyChip({ session }: PermissionPolicyChipProps) {
  ...
  const pick = (id: string): void => {
    ...
    setOpen(false)
    setFailed(false)
    setPolicy(id as PermissionPolicy)
    ...
    <Menu
      ...
      anchor={(
        <button
          ...
          aria-label={t('permissionPolicy.aria', { policy: permissionPolicyLabel(policy) })}
          aria-haspopup="menu"
          aria-expanded={open}
          onClick={() => { setOpen((v) => !v) }}
          style={chipStyle}
