...
export function installPinnedConversations(ctx: unknown): () => void {
  ...
  function ensurePinButton(row: HTMLElement, id: string): HTMLButtonElement {
    let button = row.querySelector<HTMLButtonElement>(`[data-dsh-hub-pin="${id}"]`)
    if (button === null) {
  ...
  function pruneStaleButtons(row: HTMLElement): void {
    ...
    for (const button of Array.from(row.querySelectorAll<HTMLElement>('[data-dsh-hub-pin]'))) {
  ...
  function syncPinnedSection(tree: HTMLElement): void {
    ...
    for (const id of live) {
      ...
      item.addEventListener('click', (event) => {
        if ((event.target as HTMLElement).closest(`[data-dsh-hub-pin-unpin]`) !== null) {
          event.preventDefault()
          event.stopPropagation()
          togglePin(id)
          return
