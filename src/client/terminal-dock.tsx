...
/** Minimal xterm theme shape (hex colors only — canvas renderer). */
interface TerminalThemeLike {
  background: string
  foreground: string
  cursor: string
  cursorAccent: string
...
function cssColorToHex(cssColor: string): string | null {
  ...
  const to2 = (v: string): string => Math.max(0, Math.min(255, Math.round(Number(v)))).toString(16).padStart(2, '0')
 ...
 */
function measureCell(el: HTMLElement, term: Terminal): { cw: number; ch: number } {
  const probe = document.createElement('span')
  probe.setAttribute('aria-hidden', 'true')
  probe.style.cssText = [
    'position:absolute',
...
function TabTerminal({ tabId }: { tabId: string }): ReactNode {
  ...
  useEffect(() => {
    ...
    const off = ptySubscribeData(tabId, (chunk) => { try { term.write(chunk) } catch { /* terminal disposed */ } })
...
function injectTerminalStyle(): void {
  ...
  return (
    <div ref={dockRef} style={{ ...DOCK, height: maximized ? '100vh' : height + 'px', borderRadius: maximized ? 0 : '12px 12px 0 0' }}>
      <div
        ...
        title="拖拽调整高度"
        style={{ height: 6, cursor: 'ns-resize', touchAction: 'none', flexShrink: 0, background: 'transparent', position: 'relative', zIndex: 5 }}
      />
      <div style={TABBAR}>
        {tabs.map((t) => (
      ...
      <div style={OUTER}>
        {/* Lazy mount: only the active tab gets a live xterm; switching tabs
            remounts it and ptySubscribeData replays the store ring buffer. */}
        {active !== undefined ? (
          <div key={active.id} style={TABBODY}>
            <TabTerminal tabId={active.id} />
          </div>
        ) : null}
      </div>
      {settingsOpen && (
        <div style={SETTINGS}>
          <div style={{ fontSize: 12, color: 'var(--dsw-alias-label-secondary, #bbb)' }}>终端设置</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--dsw-alias-label-tertiary, #999)' }}>字体大小</span>
