...
export const CARD_CSS_CLASSES = {
  ...
  // Collapsible setting groups (2026-08-30): resolution / general /
  // appearance each own a fold header; unmatched items sit outside.
  group: 'mg-card-group',
...
.${css.groupTitle}:focus-visible { outline: 2px auto var(--dsw-alias-brand-primary, #3964fe); }
.${css.group} > .${css.section} { padding-top: 2px; }
.${css.group}[hidden] { display: none; }
.${css.field} {
  display: flex;
  flex-direction: column;
  gap: 6px;
 ...
 */
export function injectChatVisibilityStyle(): void {
