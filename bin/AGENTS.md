...
| 文件 | 模块类别 | 职责 |
|---|---|---|
| `launcher.mjs` | **Controller** | 桌面入口：单实例锁、多实例检测、bundle 装配检查、`spawn dsh web --port 0`、崩溃自动重启（≤3 次）、quit.marker 判别 |
| `dsh-hub.mjs` | **Controller** | 终端命令入口（`dsh-hub`）：转发参数启动，继承输出 |
...
```
桌面快捷方式 → wscript launcher.vbs（隐藏控制台）→ launcher.mjs
...
6. **VBS 勿手改**：`launcher.vbs` 由 postinstall 生成（UTF-16LE BOM，含中文注释）；git 里它是模板，npm install 会重写——提交前确认无关改动则 `git checkout -- bin/launcher.vbs`。
