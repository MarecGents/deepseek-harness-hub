...
### 2.2 2026-09-04 破坏性变更审计（alpha.1 → rc.1）
- **必须修复 2 处**（升级到 rc.1 时）：
  1. **`@deepseek-ai/dsh-settings` 移除 `settingsNamespace()` 与 `installSettingsSection()`**（`packages/settings/settings/src/index.ts`，alpha.1=3 处引用、rc.1=0）——dsh-hub `src/index.ts:38` 直接 import 两者，升级后编译失败。替代：`SettingsProvider.installSection(owner, ns, schema, entry, hooks)` 实例方法；`register` 签名改为编译期小写 kebab-case 校验。
  2. **`conversation.input.left` slot 的 owner 被移除**（`packages/client/ui-conversation/src/client/contract/slots.ts:133` 声明 `{ kind:'list'; scope:'session' }` 无 owner；`InputBar.tsx:461` 渲染 `renderSlot('conversation.input.left', {})`）——dsh-hub `PermissionPolicyChip` 读 `props.session?.id`，升级后恒为 `undefined`，芯片只在挂载时取一次策略、切换会话不再刷新（功能回归，不崩溃）。

### 2.3 2026-09-05 破坏性变更审计（rc.1 → 0.1.3-alpha.1）
- **上次 2 处破坏性变更仍存在**（未修复）：`installSettingsSection`/`settingsNamespace` 仍缺失；`conversation.input.left` owner 仍为空。
- **本次新增破坏面**（dsh-hub 未使用，升级时需注意）：
  - **Session v2**：`assistant/chunk` 事件从 known-event-types 移除，改为 `assistant/attempt` + 内嵌 `stream: AssistantStreamRecord[]`（`{ type:'chunk', chunk:{ type:'usage', usage } }` 承载用量）；`assistant/message` 新增必填 `stream` 字段。**dsh-usage-stats 插件使用 `assistant/chunk`，升级后需迁移**。
  - **图片→通用附件重命名**：`conversation.input.attachments` owner props 改名（`onAddImages`→`onAddFiles` 等）；`SubmitImageAttachment`→`SubmitAttachment`；`InputState.imageIds`→`attachmentIds`。
  - **`MessageText` 从 ui-primitives 删除**（ui-goal 改用 `projectUserText`）。
  - **`ConnectionFetchHandler` 新增必填 `requestBodyMode()`**；`ConnectionFetchRoute` 新增必填 `requestBody`。
  - **`ChatNodeOwnerProps`/`ToolCallOwnerProps` 新增必填 `loadImage`**；新增 `tool.call.images` slot。
  - 新包 `@deepseek-ai/dsh-client-file-upload`（0.1.3-alpha.1）。
- **不受影响**：`ctx.webServer` 四方法、`slots.inject/register`、`settings.section`、`dsh.bundle.patch`/`dsh.profile.bundles`、Cordis 4.0.2、事件 `session/event`/`sandbox/mode`/`agent/*`。
- **⚠️ 关键结论（2026-09-05 实测）**：**npm registry 最新发布版是 0.1.2-rc.1**（0.1.3-alpha.1 尚未发布 npm）；用户实际运行的 dsh 0.1.1-rc.2 中 `installSettingsSection`/`settingsNamespace` 仍存在、`conversation.input.left` 仍传 session owner、`assistant/chunk` 仍存在——**dsh-hub 当前与运行环境完全兼容，无需修复**。上述破坏性变更仅在升级到 0.1.2-rc.1+ 时触发。
- **必须升版本**：dsh-hub 全部 `@deepseek-ai/*` peer/devDependencies 为 `^0.1.0-rc.6`，按 semver `^0.1.0-rc.6` 不匹配 `0.1.2-rc.1`（prerelease 需同 [major,minor,patch] 元组），需统一升至 `^0.1.2-rc.1`。
- **不受影响**：`ctx.webServer` 四方法签名、`settings.section` slot（order-based）、`slots.inject/register`、事件系统（session/event、sandbox/mode、turn/end）、profile 装配（cordis.patch.yml/dsh.bundle.patch/dsh.profile.bundles）、Cordis 4.0.1→4.0.2（src 零 diff）、ui-primitives 全部在用 Icon/Menu/Toast 保留。
- **新增**：`keyedHooks` 注入面、`IconDatabaseOutline16`/`IconClockOutline16`/`IconAlarmClockOutline16`、`ConnectionIndicator`（替换 ConnectionBanner）、`diffTotals`。
