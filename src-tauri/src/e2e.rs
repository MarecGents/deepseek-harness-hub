...
fn run_e2e(win: WebviewWindow) {
    ...
    // ── ④ 自绘托盘菜单（真实路径：tray.rs 右键 → win.eval __mgTrayMenuToggle）──
    info!("e2e: invoking __mgTrayMenuToggle (same JS the tray right-click runs)");
    ...
    let _ = win.eval(
        r#"(function(){
            ...
            window.__TAURI_INTERNALS__.invoke('diag_report', {
                msg: 'E2E:tray-menu open=' + open + ' items=' + items + ' bg=' + bg
            }).catch(function(){});
        })();"#,
    );
    sleep(1);
    ...
    let _ = win.eval(
        r#"(function(){
            ...
            var open = m !== null && m.classList.contains('mg-open');
            window.__TAURI_INTERNALS__.invoke('diag_report', {
                msg: 'E2E:tray-toggle open=' + open
            }).catch(function(){});
        })();"#,
    );
    sleep(1);

    // 重新打开 → 点击「打开工作区」→ 断言 mg:shell-command CustomEvent 派发（同页）。
    let _ = win.eval("window.__mgTrayMenuToggle && window.__mgTrayMenuToggle()");
    sleep(1);
    let _ = win.eval(
        r#"(function(){
...
/// 标题栏皮肤跟随真实断言：强制浅色态 → 读背景/文字色；强制深色态 → 再读；
/// 最后恢复初始标记。transition 0.2s，每次切换等 400ms 稳定到目标值。
        ...
        r#"(function(){
            ...
            setTimeout(function(){
                ...
                setTimeout(function(){
                    ...
                    window.__TAURI_INTERNALS__.invoke('diag_report', {
                        msg: 'E2E:titlebar light=' + light + ' lc=' + lightColor + ' dark=' + dark + ' dc=' + darkColor
                    }).catch(function(){});
                }, 400);
            }, 400);
        })();"#,
