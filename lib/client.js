...
`;
		...
		* search-result rows (`<button role="treeitem">`). */
		...
		const PIN_FILLED_SVG = `<svg class="${PIN_CSS_CLASSES.pinSvg}" width="14" height="14" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="${PIN_PATH}"/></svg>`;
		const PIN_OUTLINE_SVG = `<svg class="${PIN_CSS_CLASSES.pinSvg}" width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><path d="${PIN_PATH}"/></svg>`;
		...
		function installPinnedConversations(ctx) {
			...
			*/
			...
			function syncPinnedSection(tree) {
				...
				let section = Array.from(slot.children).find((el) => el.classList.contains(PIN_CSS_CLASSES.section));
				if (section === void 0) {
				...
				if (section.parentNode !== slot || section.nextSibling !== tree) slot.insertBefore(section, tree);
