		...
		function installConversationRail(ctx) {
			...
			*/
			...
			function textOfContent(content) {
				...
				return first && first.type === "text" && typeof first.text === "string" ? first.text.trim().slice(0, 60) : "";
			}
			function textOfBlocks(blocks) {
				const first = Array.isArray(blocks) ? blocks[0] : void 0;
				return first && first.kind === "text" && typeof first.text === "string" ? first.text.trim().slice(0, 60) : "";
			}
			function turnPreviews(snapshot) {
				const out = [];
				let cur = -1;
				for (const node of snapshot?.nodes ?? []) {
					...
					const t = node.kind === "user" ? textOfContent(node.content) : node.kind === "assistant" ? textOfBlocks(node.blocks) : "";
					if (t) out[cur] = t;
				}
				return out;
			}
