window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region ../marec-dsh-desktop/node_modules/clsx/dist/clsx.mjs
		function r(e) {
			var t, f, n = "";
...
`;
		...
		*/
		function applyBackground(backgroundId) {
			...
			style.textContent = `body{background-image:url("${background.url}") !important;background-size:cover !important;background-position:center !important;background-repeat:no-repeat !important;background-attachment:fixed !important;}`;
		}
...
/* Rename action: small text button on hover (no custom icon needed). */
.${css$1.itemRename} {
  position: absolute;
  right: 34px;
  top: 50%;
  ...
  cursor: pointer;
  opacity: 0;
  transition: opacity 0.12s var(--ds-ease-in-out, ease), background 0.12s, color 0.12s;
}
.${css$1.item}:hover .${css$1.itemRename},
.${css$1.itemRename}:focus-visible { opacity: 1; }
.${css$1.itemRename}:hover {
  background: var(--dsw-alias-interactive-bg-hover, rgb(0 0 0 / 6%));
  color: var(--dsw-alias-state-business-primary, #3964fe);
}
.${css$1.itemRename}:focus-visible {
  outline: 2px solid var(--dsw-alias-brand-primary, #3964fe);
  outline-offset: -1px;
}
		...
		function openSessionMenu(params) {
			...
			const entries = [
				...
				workspacePath === void 0 ? null : {
					label: "在资源管理器中打开",
					run: () => {
						fetchJson("/api/dsh-hub/session/explorer", {
							method: "POST",
							headers: { "content-type": "application/json" },
				...
				},
				{
					label: "复制日志路径",
					run: () => {
						fetchJson(`/api/dsh-hub/session/paths?${new URLSearchParams({ id: params.id })}`).then((d) => {
			...
			function syncPinnedSection(tree) {
				...
				for (const id of live) {
					...
					if (editingId === id) {
						...
						rename.type = "button";
						rename.className = PIN_CSS_CLASSES.itemRename;
