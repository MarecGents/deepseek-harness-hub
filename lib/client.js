		...
		*/
		...
		function handleShellCommand(ctx, event) {
			...
			if (detail?.command === "open-workspace") {
				...
				if (ipc) ipc.postMessage(`mg:workspace-path:${path === void 0 ? "" : encodeURIComponent(path)}`);
				else try {
					window.__TAURI_INTERNALS__?.event?.emit?.("mg:workspace-path", { path: path ?? "" }).catch?.(() => {});
				} catch {}
			}
		}
		/** Client plugin body. */
