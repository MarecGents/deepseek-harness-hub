 ...
 */
import {} from '../models/shell-config.js';
...
import { applySecureHeaders } from "./security-headers.js";
            ...
            handler: (req, res) => {
                ...
                if (req.method === 'POST') {
                    ...
                    return readJsonBody(req).then((body) => {
                        ...
                        if (typeof record.skin === 'string' && /^[a-z0-9-]{1,64}$/i.test(record.skin))
                            patch.skin = record.skin;
                        // Background id — same whitelist rationale (served via
                        // /api/dsh-hub/backgrounds/<id>).
                        if (typeof record.background === 'string' && /^[a-z0-9-]{1,64}$/i.test(record.background))
                            patch.background = record.background;
                        // Desktop-icon id — same whitelist rationale (icons/<id>.ico lookup).
                        if (typeof record.desktopIcon === 'string' && /^[a-z0-9-]{1,64}$/i.test(record.desktopIcon))
                            patch.desktopIcon = record.desktopIcon;
