...
export function createPty(cwd, cols = 100, rows = 28) {
    const id = 'pty-' + randomUUID().slice(0, 8);
    const shell = 'PowerShell';
    const tab = { id, shell, cwd, title: titleFor(cwd), createdAt: Date.now(), alive: true };
    ...
    const session = { tab, pty, buffer: '', subscribers: new Set() };
    pty.onData((data) => {
        session.buffer += data;
    ...
    pty.onExit(() => { tab.alive = false; for (const cb of session.subscribers)
