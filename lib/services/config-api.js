            ...
            handler: (req, res) => {
                ...
                if (req.method === 'POST') {
                    return readJsonBody(req).then((body) => {
                        ...
                        if (typeof record.width === 'number' && record.width >= 480)
                            patch.width = Math.floor(record.width);
                        if (typeof record.height === 'number' && record.height >= 360)
                            patch.height = Math.floor(record.height);
