 ...
 */
function setNativeTitleBarTheme(hwnd, dark) {
    try {
        const script = [
...
export function openDesktopShell(port, options, onExit) {
    ...
    const applyWindowTheme = (w, wv, theme) => {
        ...
        if (theme === 'system') {
            ...
            if (hwnd !== undefined)
                setNativeTitleBarTheme(hwnd, true);
            ...
            detector.start((dark) => {
                ...
                if (hwnd !== undefined)
                    setNativeTitleBarTheme(hwnd, dark);
            });
        }
        else {
     ...
     */
    const dispatchScript = (name, detail) => `window.__marecShellReady === true`
