        ...
        ctx.effect(() => {
            ...
            const onContext = (event) => {
                ...
                if (target instanceof Element) {
                    ...
                    if (target.closest('div[data-slot="sidebar.workspaces"], [role="tree"], div[role="treeitem"]'))
                        return;
                    // Interactive controls keep their native/own behavior (no menu now
                    // that the WebView2 menu is disabled) — never the refresh item.
                    if (target.closest('textarea, input, select, button, a, [contenteditable="true"], [role="button"]'))
                        return;
                }
                event.preventDefault();
                closeMenu();
