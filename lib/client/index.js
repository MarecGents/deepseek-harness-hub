...
import { AppearanceCenterSection } from "./settings-card.js";
...
import { installConversationRail } from "./conversation-rail.js";
import { TerminalPage } from "./terminal-dock.js";
import { SessionTabs } from "./SessionTabs.js";
import { ptyOpen, ptyToggle } from "./pty-store.js";
...
export function apply(ctx) {
    ...
    try {
        ...
        // Ctrl+J toggles the terminal panel (ZCode-style).
        ctx.effect(() => {
            const onKey = (event) => {
    ...
    // Standalone terminal page (独立终端页): a full-screen overlay opened from
    // the left sidebar's 终端 entry. Replaces the earlier floating bottom dock.
    try {
        ctx.effect(() => {
            const host = document.createElement('div');
    ...
    try {
        slots.inject('sidebar.footer.action', function* () {
            ...
            }, createElement(IconCodeOutline16, { size: 16 }), props.wide ? '终端' : null));
        });
    }
    catch (error) {
