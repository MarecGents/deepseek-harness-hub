...
import { IconCodeOutline16, IconFolderClose16, IconFolderOpen16, IconPanelLeftOutline16, } from '@deepseek-ai/dsh-client-ui-primitives';
...
function TreeNode({ entry, depth }) {
    ...
    return (_jsxs("li", { children: [_jsxs("div", { className: c.treeRow, style: { paddingLeft: `${depth * 12 + 4}px` }, onClick: () => { if (expandable)
                    setOpen(!open); }, children: [_jsx("span", { className: c.treeIcon, children: expandable
                            ? (open ? _jsx(IconFolderOpen16, { size: 14 }) : _jsx(IconFolderClose16, { size: 14 }))
                            : _jsx(IconCodeOutline16, { size: 14 }) }), _jsx("span", { className: c.treeName, children: entry.name })] }), open && children !== null && (_jsx("ul", { className: c.treeChildren, children: children.map((child) => _jsx(TreeNode, { entry: child, depth: depth + 1 }, child.path)) }))] }));
}
export function RightSidebar({ ctx, useSessions, useWorkspaces }) {
    ...
    return (_jsx("div", { className: clsx(c.root, !open && c.collapsed), style: { width: open ? 360 : 56 }, children: open ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: c.header, children: [_jsxs("div", { className: c.headerTop, children: [_jsx("span", { className: c.title, children: "\u53F3\u4FA7\u680F" }), _jsx("button", { type: "button", className: c.toggle, "aria-label": "\u6536\u8D77\u53F3\u4FA7\u680F", onClick: () => { setOpen(false); }, children: _jsx(IconPanelLeftOutline16, { className: c.toggleIcon, size: 16 }) })] }), _jsx("div", { className: c.tabs, role: "tablist", "aria-label": "\u53F3\u4FA7\u680F\u89C6\u56FE", children: ['overview', 'files', 'git'].map((key) => (_jsx("button", { type: "button", role: "tab", "aria-selected": tab === key, className: clsx(c.tab, tab === key && c.tabActive), onClick: () => { setTab(key); }, children: key === 'overview' ? '概览' : key === 'files' ? '文件' : 'Git' }, key))) })] }), _jsx("div", { className: c.body, children: _jsxs("div", { className: c.content, children: [tab === 'overview' && (_jsx(Overview, { totalInputTokens: totalInputTokens, totalOutputTokens: totalOutputTokens, totalTokens: totalTokens, chartGradient: chartGradient, stats: stats, usage: usage, turnTokens: turnTokens, fileCount: rootEntries.filter((e) => e.isFile).length, dirCount: rootEntries.filter((e) => e.isDirectory).length, git: git, loading: workspaceLoading })), tab === 'files' && (_jsxs("div", { className: c.section, children: [_jsxs("div", { className: c.sectionTitle, children: ["\u5DE5\u4F5C\u533A\u6587\u4EF6", effectivePath !== '' && (_jsx("button", { type: "button", className: c.refresh, onClick: () => { refreshWorkspace(); }, children: "\u5237\u65B0" }))] }), effectivePath === ''
                                        ...
                                        : workspaceLoading && rootEntries.length === 0
                                            ...
                                            : rootEntries.length === 0
                                                ...
                                                : (_jsx("ul", { className: c.tree, children: rootEntries.map((entry) => _jsx(TreeNode, { entry: entry, depth: 0 }, entry.path)) }))] })), tab === 'git' && (_jsx(GitTab, { git: git, loading: workspaceLoading }))] }) })] })) : (_jsxs("div", { className: c.rail, children: [_jsx("button", { type: "button", className: c.toggle, "aria-label": "\u5C55\u5F00\u53F3\u4FA7\u680F", onClick: () => { setOpen(true); }, children: _jsx(IconPanelLeftOutline16, { className: c.toggleIcon, size: 18 }) }), _jsxs("div", { className: c.railItems, children: [_jsx("span", { className: c.railPlaceholder, "aria-hidden": true }), _jsx("span", { className: c.railPlaceholder, "aria-hidden": true }), _jsx("span", { className: c.railPlaceholder, "aria-hidden": true })] })] })) }));
}
