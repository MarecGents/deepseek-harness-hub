...
import { IconBranchOutline16, IconCheckOutline16, IconCodeOutline16, IconCopyOutline16, IconDataOutline16, IconDownloadOutline16, IconEditOutline16, IconFolderClose16, IconFolderOpen16, IconLinkOutline16, IconPanelLeftOutline16, IconPlayOutline16, IconRightUpOutline16, IconSendOutline16, IconThinkOutline16, } from '@deepseek-ai/dsh-client-ui-primitives';
import { RIGHT_SIDEBAR_CSS_CLASSES as c } from "./right-sidebar-style.js";
import { ptyOpen, ptyRetarget } from "./pty-store.js";
...
async function openFolderAsWorkspace(ctx) {
    ...
    const p = window.__mgGetCurrentWorkspace?.() || '';
    void openInOs(p);
}
...
function insertReferenceIntoComposer(ctx, sessionId, text) {
    ...
    if (insertReferenceIntoComposerDom(text))
        return;
...
function TreeNode({ entry, depth, onContext }) {
    ...
    return (_jsxs("li", { children: [_jsxs("div", { className: c.treeRow, style: { paddingLeft: `${depth * 12 + 4}px` }, onClick: () => { if (expandable)
                    setOpen(!open); }, onContextMenu: (event) => { event.preventDefault(); event.stopPropagation(); onContext(entry, event); }, "data-row-path": entry.path, "data-row-dir": expandable ? '1' : '0', children: [_jsx("span", { className: c.treeIcon, children: expandable
                            ? (open ? _jsx(IconFolderOpen16, { size: 16 }) : _jsx(IconFolderClose16, { size: 16 }))
                            : _jsx(IconCodeOutline16, { size: 16 }) }), _jsx("span", { className: c.treeName, children: entry.name })] }), open && children !== null && (_jsx("ul", { className: c.treeChildren, children: children.map((child) => _jsx(TreeNode, { entry: child, depth: depth + 1, onContext: onContext }, child.path)) }))] }));
}
...
function ContextMenu({ x, y, entry, ctx, sessionId, root, onClose }) {
    ...
    if (entry.isDirectory) {
        items.push({ label: '打开（展开/折叠）', icon: _jsx(IconFolderOpen16, { size: 14 }), run: toggleDir });
        items.push({ label: '路径引用', icon: _jsx(IconLinkOutline16, { size: 14 }), run: () => { insertReferenceIntoComposer(ctx, sessionId, reference); } });
        items.push({ label: '复制目录', icon: _jsx(IconCopyOutline16, { size: 14 }), run: () => { copyText(entry.path); } });
        items.push({ label: '在此打开终端', icon: _jsx(IconCodeOutline16, { size: 14 }), run: () => { void ptyOpen(entry.path); } });
    }
    else {
        items.push({ label: '打开', icon: _jsx(IconPlayOutline16, { size: 14 }), run: () => { void openInOs(entry.path); } });
        items.push({ label: '路径引用', icon: _jsx(IconLinkOutline16, { size: 14 }), run: () => { insertReferenceIntoComposer(ctx, sessionId, reference); } });
        items.push({ label: '复制文件夹目录', icon: _jsx(IconCopyOutline16, { size: 14 }), run: () => { copyText(parentDir(entry.path)); } });
        items.push({ label: '复制文件路径', icon: _jsx(IconCopyOutline16, { size: 14 }), run: () => { copyText(entry.path); } });
        items.push({ label: '在此打开终端', icon: _jsx(IconCodeOutline16, { size: 14 }), run: () => { void ptyOpen(parentDir(entry.path)); } });
    }
    const MENU_STYLE = {
        position: 'fixed',
        left: x,
    ...
    return (_jsx("div", { "data-rs-menu": true, style: MENU_STYLE, children: items.map((item) => (_jsxs("button", { type: "button", style: ITEM_STYLE, onClick: () => { item.run(); onClose(); }, children: [item.icon, _jsx("span", { children: item.label })] }, item.label))) }));
}
export function RightSidebar({ ctx }) {
    ...
    useEffect(() => {
        ...
        const onPointerDown = (event) => {
            ...
            if (target !== null && target.closest && target.closest('[data-rs-menu]') !== null)
                return;
            setMenu(null);
        };
    ...
    useEffect(() => {
        if (effectivePath !== '')
            void ptyRetarget(effectivePath);
    }, [effectivePath]);
    ...
    return (_jsxs("div", { className: clsx(c.root, !open && c.collapsed), style: { width: open ? 360 : 56 }, children: [open ? (_jsxs(_Fragment, { children: [_jsxs("div", { style: { display: 'flex', gap: 8, padding: '6px 10px', alignItems: 'center', borderBottom: '1px solid var(--dsw-alias-border-l2, rgb(0 0 0 / 10%))' }, children: [_jsxs("button", { type: "button", style: TOP_BTN, onClick: () => { void openFolderAsWorkspace(ctx); }, children: [_jsx(IconFolderOpen16, { size: 14 }), " \u6253\u5F00\u5DE5\u4F5C\u533A\u6587\u4EF6\u5939"] }), _jsxs("button", { type: "button", style: TOP_BTN, onClick: () => { void ptyOpen(effectivePath || undefined); }, children: [_jsx(IconCodeOutline16, { size: 14 }), " \u7EC8\u7AEF"] })] }), _jsxs("div", { className: c.header, children: [_jsx("div", { className: c.tabs, role: "tablist", "aria-label": "\u53F3\u4FA7\u680F\u89C6\u56FE", children: ['overview', 'files', 'git'].map((key) => (_jsx("button", { type: "button", role: "tab", "aria-selected": tab === key, className: clsx(c.tab, tab === key && c.tabActive), onClick: () => { setTab(key); }, children: key === 'overview' ? '概览' : key === 'files' ? '文件' : 'Git' }, key))) }), _jsx("button", { type: "button", className: c.toggle, "data-tip": "\u6536\u8D77\u4FA7\u8FB9\u680F", "aria-label": "\u6536\u8D77\u53F3\u4FA7\u680F", onClick: () => { setOpen(false); }, children: _jsx(IconPanelLeftOutline16, { className: c.toggleIcon, size: 16 }) })] }), _jsx("div", { className: c.body, children: _jsxs("div", { className: c.content, children: [tab === 'overview' && (_jsx(Overview, { totalInputTokens: totalInputTokens, totalOutputTokens: totalOutputTokens, totalTokens: totalTokens, chartGradient: chartGradient, stats: stats, usage: usage, turnTokens: turnTokens, fileCount: rootEntries.filter((e) => e.isFile).length, dirCount: rootEntries.filter((e) => e.isDirectory).length, git: git, loading: workspaceLoading })), tab === 'files' && (_jsxs("div", { className: c.section, children: [_jsxs("div", { className: c.sectionTitle, children: ["\u5DE5\u4F5C\u533A\u6587\u4EF6", effectivePath !== '' && (_jsx("button", { type: "button", className: c.refresh, onClick: () => { refreshWorkspace(); }, children: "\u5237\u65B0" }))] }), effectivePath === ''
                                            ...
                                            : workspaceLoading && rootEntries.length === 0
                                                ...
                                                : rootEntries.length === 0
                                                    ...
                                                    : (_jsx("ul", { className: c.tree, children: rootEntries.map((entry) => _jsx(TreeNode, { entry: entry, depth: 0, onContext: onRowContext }, entry.path)) }))] })), tab === 'git' && (_jsx(GitTab, { git: git, loading: workspaceLoading }))] }) })] })) : (_jsxs("div", { className: c.rail, children: [_jsx("button", { type: "button", className: c.toggle, "data-tip": "\u5C55\u5F00\u4FA7\u8FB9\u680F", "aria-label": "\u5C55\u5F00\u53F3\u4FA7\u680F", onClick: () => { setOpen(true); }, children: _jsx(IconPanelLeftOutline16, { className: c.toggleIcon, size: 18 }) }), _jsxs("div", { className: c.railItems, children: [_jsx("button", { type: "button", className: c.railItem, "data-tip": "\u6982\u89C8", "aria-label": "\u6982\u89C8", onClick: () => { setTab('overview'); setOpen(true); }, children: _jsx(IconDataOutline16, { size: 18 }) }), _jsx("button", { type: "button", className: c.railItem, "data-tip": "\u6587\u4EF6", "aria-label": "\u6587\u4EF6", onClick: () => { setTab('files'); setOpen(true); }, children: _jsx(IconFolderOpen16, { size: 18 }) }), _jsx("button", { type: "button", className: c.railItem, "data-tip": "Git", "aria-label": "Git", onClick: () => { setTab('git'); setOpen(true); }, children: _jsx(IconBranchOutline16, { size: 18 }) })] })] })), menu !== null && (_jsx(ContextMenu, { x: menu.x, y: menu.y, entry: menu.entry, ctx: ctx, sessionId: currentSessionId, root: effectivePath, onClose: closeMenu }))] }));
}
