...
function save() {
    ...
    catch { /* Storage full/blocked — the host copy below still persists. */ }
 ...
 */
export async function syncHostPrefs() {
    const token = globalThis.__DSH_HUB_TOKEN__ ?? '';
    if (token === '')
        return;
    try {
        ...
        if (body.ok !== true || body.prefs === undefined)
            return;
        const p = body.prefs;
        const next = {
