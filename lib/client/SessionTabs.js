...
const TAB = {
    ...
    // Tab cards must read against the titlebar (which paints bg-layer-1):
    // inactive tabs get a faint lift (30% of the skin's layer-3 = the
    // floating/card surface), the active tab goes nearly solid (85%) plus the
    // brand underline — the strip stays clearly bounded instead of melting
    // into the bar (2026-08-30 report: tabs were invisible until the 2px line).
    background: 'color-mix(in srgb, var(--dsw-alias-bg-layer-3, #2a2f3a) 30%, transparent)',
    border: 'none', color: 'var(--dsw-alias-label-tertiary, #9aa7bd)', whiteSpace: 'nowrap',
...
const TAB_ACTIVE = {
    ...TAB,
    // Active tab: OPAQUE brand-tinted card, strong enough to read against a
    //   white titlebar. In the DEFAULT skin + light mode every bg-layer token
    //   is white, so 10% brand was invisible (audit v4); 22% brand over the
    //   layer-3 surface gives a clear pale-blue card on light and a blue-grey
    //   card on dark — the tab region is unmistakably separated from the bar
    //   in BOTH modes (2026-09-01 audit v5, user-verified criteria).
...
const PLUS = { border: 'none', background: 'transparent', color: 'inherit', cursor: 'pointer', fontSize: 15, padding: '0 7px', borderRadius: 6, flex: 'none', height: '100%', WebkitAppRegion: 'no-drag' };
const INLINE_INPUT = {
                ...
                return (_jsxs("div", { "data-tab-id": id, role: "tab", "aria-selected": active, draggable: true, title: s?.title ?? id, style: tabStyle, onClick: () => { if (!editing)
                        open(id); }, onContextMenu: (e) => onContextMenu(id, e), onDragStart: onDragStart(id), onDragOver: onDragOver(id), onDrop: onDrop, onDragEnd: onDragEnd, children: [st !== null && (_jsx("span", { style: { width: 7, height: 7, borderRadius: '50%', background: st.color, flex: 'none', display: 'inline-block', ...(st.pulse ? { animation: 'mgTabStatusPulse 1.1s ease-in-out infinite' } : {}) }, title: st.title })), editing ? (_jsx("input", { autoFocus: true, value: draft, onChange: (e) => setDraft(e.target.value), onBlur: () => commitRename(id), onKeyDown: (e) => {
                                ...
                                if (e.key === 'Enter')
                                    ...
                                    cancelRename();
                            }, onClick: (e) => e.stopPropagation(), style: { ...INLINE_INPUT, width: Math.max(80, draft.length * (draft.length && /[\u4e00-\u9fff]/.test(draft) ? 12 : 7) + 22) } })) : (_jsx("span", { style: { flex: '1', textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }, children: titleOf(id) })), _jsx("span", { role: "button", "aria-label": "\u5173\u95ED\u6807\u7B7E", title: "\u5173\u95ED\u6807\u7B7E", style: { color: 'var(--dsw-alias-label-tertiary, #888)', padding: '0 3px', borderRadius: 4, cursor: 'pointer', lineHeight: '14px', flex: 'none' }, onClick: (e) => { e.stopPropagation(); close(id); }, children: "\u00D7" })] }, id));
            })] }));
