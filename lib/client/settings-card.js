...
export function DesktopSettingsCard(_props) {
    ...
    const groupCard = (key, title, description, body) => (_jsxs("li", { className: clsx(c.card, groupOpen[key] && c.cardOpen), children: [_jsxs("button", { type: "button", className: c.header, "aria-expanded": groupOpen[key], "aria-label": `${groupOpen[key] ? '收起' : '展开'}: ${title}`, onClick: () => setGroupOpen((g) => ({ ...g, [key]: !g[key] })), children: [_jsxs("span", { className: c.headText, children: [_jsx("span", { className: c.name, children: title }), _jsx("span", { className: c.description, children: description })] }), dirty ? _jsx("span", { className: c.pending, children: COPY.unsaved }) : null, _jsx(IconChevronDownOutline14, { className: clsx(c.chevron, groupOpen[key] && c.chevronOpen) })] }), groupOpen[key] ? _jsx("div", { className: c.body, children: body }) : null] }));
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: 10 }, children: [loading
                ? _jsx("div", { className: c.loading, role: "status", "aria-label": "\u8BFB\u53D6\u914D\u7F6E\u2026" })
                : (_jsxs(_Fragment, { children: [draft !== null && groupCard('resolution', COPY.resolutionSection, t('settings.resolutionDesc'), (_jsxs("div", { className: c.section, children: [_jsxs("div", { className: c.field, children: [_jsx("span", { className: c.fieldLabel, children: COPY.widthLabel }), _jsx("input", { className: c.input, type: "number", min: 480, max: Math.floor(window.screen.width), "aria-label": COPY.widthLabel, value: draft.width, onChange: (event) => {
