...
import { hasStoredWindowSize, makeConfigRoutes, migrateLegacyPaths, readShellConfig } from './services/config-api.js';
...
function currentWorkspacePath(ctx) {
    try {
        ...
        if (first?.path !== undefined && first.path !== '')
            return first.path;
    }
    catch {
        // Fall through.
...
function currentWorkspaceId(ctx) {
    try {
        const registry = ctx.get('workspaceRegistry');
 ...
 */
async function openWorkspaceDir(ctx) {
    try {
 ...
 */
function newTaskInWeb(ctx, dispatch) {
    try {
...
function effectiveConfig(config) {
    ...
    return {
        ...config,
        width: sized ? stored.width : undefined,
        height: sized ? stored.height : undefined,
