...
import { openDesktopShell } from "./managers/desktop.js";
import { openDesktopShellTauri } from "./managers/tauri-shell.js";
import { makeBridgeRoutes } from "./server/bridge-server.js";
import { makeConfigRoutes, hasStoredWindowSize, migrateLegacyPaths, readShellConfig, storedNotifyOnTaskComplete, storedSoundEnabled } from './server/config-api.js';
import { setAppUserModelId } from './helpers/app-id.js';
import { dshHome } from './helpers/state-store.js';
import { openFolderInExplorer } from './helpers/explorer.js';
...
import { TrayCommandRegistry } from "./core/registry.js";
 ...
 */
function getFocusedSessionState(ctx) {
    try {
    ...
    const open = () => {
        ...
        if (isTauriShell()) {
            ...
            if (h) {
                ...
                // 托盘命令注册表路由（SPT 思想）：新增命令 = register，不改分发链。
                const trayCommands = new TrayCommandRegistry();
                trayCommands.register('open-workspace', (ctx) => {
                ...
                trayPipe.on('line', (line) => {
                    if (!line.startsWith('MG_TRAY '))
                        return;
                    let payload;
                    try {
                        payload = JSON.parse(line.slice('MG_TRAY '.length));
                    }
                    catch {
                    ...
                    try {
                        if (!trayCommands.dispatch(command, ctx, payload)) {
