...
export function apply(ctx, config) {
    ...
    const legacyInstallSection = ctx.settings.installSection;
    ...
    const open = () => {
        ...
        if (h) {
            ...
            if (effective.width && effective.height) {
                // Boot sync yields to a maximized window: the shell restores the
                // maximized state during setup (lib.rs restore_window_state); without
                // allowUnmaximize:false this unconditional resize used to un-maximize
                // it again right after launch (2026-09-06 fix). Manual saves from the
                // settings card go through the config onChange path below, which keeps
                // the legacy unmaximize-first behavior.
                h.applySize(effective.width, effective.height, { allowUnmaximize: false });
            }
        }
        else {
