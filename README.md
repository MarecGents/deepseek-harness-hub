...
| 分支 | 状态 | 说明 |
|---|---|---|
| `main` | 发布分支（**唯一合并目标**） | 已合并 dev-v2 → **`0.1.3`**（与 dev-v2 同步） |
| `dev-v1` | **永久冻结**（WebView2 时代存档） | 不再接收任何更新/同步；历史收尾 = `0.0.1-rc.13/rc.14`（WebView2 最终版） |
| `dev-v2` | **唯一开发分支**（Tauri 2.x 壳） | 所有更新在此开发，merge 只到 main；当前 = `0.1.3`（已 merge 进 main；npm latest/rc = 0.1.3） |
