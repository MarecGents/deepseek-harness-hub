...
> 版本：v1.1 · 2026-08-23 · 状态：生效（基于 #24–#27 与 #33–#38 两轮真实 PR 处理经验固化；#33-38 含「数据源虚构」#35 与「硬依赖 inject 阻塞」审查发现）

---

## 0. 核心原则（为什么需要这套流程）

外部 PR 常见三类问题（#24–#27 实证，同类在 #33–#38 再现）：
...
对每个值得调研的 PR（open 且非明显垃圾）产出 5 份文档，文件名带 `PR<N>[-PR<M>]-<类型>-<YYYY-MM-DD>.md`（一个调研批次可含多 PR，如 #33–#38 一套五件套）：
...
- 手动（dev-v2 通用）：首启出"启动中"→ 自动进 dsh UI；托盘四菜单；置顶/皮肤/背景/右侧栏；会话通知 + 提示音；多实例门禁；rail 自适应配色；桌面图标 6 面同步（含标题栏同源 PNG）；快速切换图标不卡死；模型嵌套菜单（#33）。
