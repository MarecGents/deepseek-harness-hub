...
//
// 职责：断言 tauri.conf.json 关键字段 + lib.rs 窗口参数符合 M1 验收；任一 FAIL exit 1。
// 模块类别：Helper（M1 验收脚本）
...
const checks = [
  ...
  ['version = 0.0.0（Cargo 恒 0.0.0）', conf.version === '0.0.0'],
  ['package.json version 为事实源（非空）', typeof pkg.version === 'string' && pkg.version.length > 0],
  ['lib.rs decorations(false)', libSrc.includes('.decorations(false)')],
  ['lib.rs transparent(false)', libSrc.includes('.transparent(false)')],
  ['lib.rs min_inner_size(480.0, 360.0)', libSrc.includes('.min_inner_size(480.0, 360.0)')],
