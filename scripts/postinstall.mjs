#!/usr/bin/env node
/**
 * postinstall.mjs — runs after `npm install marec-dsh-desktop`:
...
function installDsh() {
  console.log('[marec-dsh-desktop] dsh CLI not found; installing @deepseek-ai/dsh globally…')
...
try {
  ...
  if (found !== null && dshWorks(found)) {
    console.log(`[marec-dsh-desktop] dsh prerequisite OK (${found})`)
  } else if (installDsh()) {
    console.log('[marec-dsh-desktop] dsh installed; start it with the desktop shortcut.')
  } else {
    console.error('[marec-dsh-desktop] could not install dsh automatically. Run manually: npm install -g @deepseek-ai/dsh')
  }
} catch (error) {
  console.error('[marec-dsh-desktop] dsh prerequisite check failed:', error)
}

// ── 2. Desktop shortcut (Windows only) ──────────────────────────────────────

if (process.platform !== 'win32') {
  console.log('[marec-dsh-desktop] desktop shortcut only created on Windows; skipping.')
  process.exit(0)
}
...
function writeLauncherWrapper() {
  ...
  const content = `' Auto-generated by marec-dsh-desktop postinstall — do not edit.
...
function createShortcut() {
  ...
  console.log(`[marec-dsh-desktop] desktop shortcut created: ${result.stdout.trim()}`)
}

try {
  createShortcut()
} catch (error) {
  console.error('[marec-dsh-desktop] could not create the desktop shortcut:', error)
}
