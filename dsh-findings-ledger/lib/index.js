...
export function apply(ctx) {
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    presentCall: () => 'Capture findings baseline',
  })))
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    presentCall: () => 'Generate findings report',
  })))
  ...
  const off = ctx.on('session/event', (session, event) => {
    ...
    if (type === 'turn/start' && !existsSync(baselinePath(cwd))) {
      try { captureBaseline(cwd); baselineByCwd.set(cwd, true) } catch {}
    } else if (/turn\/end|turn\/complete|session\/end/.test(type)) {
