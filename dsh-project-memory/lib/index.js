/**
 * @dsh-external/dsh-project-memory — 每项目持久记忆（对标 Zcode 的 .reasonix / FACT.md + JOURNAL.jsonl）
 *
 * 能力：
 *  1. 自动加载  systemPrompt.context 每次模型步把当前工作区 .dsh-memory/FACT.md + JOURNAL.jsonl 尾部
 *     注入运行时上下文（session 启动即生效，无需显式读文件）。
 *  2. 工具写回  memory_read / memory_log / memory_fact（写 FACT.md、追加 JSONL、去重）。
 *  3. 自动沉淀  session/event 监听：turn 结束时追加 breadcrumb 到 JOURNAL.jsonl。
 *
 * 存储：<workspace>/.dsh-memory/（隐藏目录，随项目走，git 可忽略）。
 * 零外部运行时依赖（只 node 内建模块）——不 import @deepseek-ai/*，可脱离 junction 依赖直接加载。
 */
import { readFileSync, writeFileSync, appendFileSync, mkdirSync, existsSync, statSync, openSync, readSync, closeSync } from 'node:fs'
...
function renderMemory(cwd) {
  if (!cwd || !existsSync(memoryDir(cwd))) return ''
  ...
  return '### 项目记忆（dsh-project-memory，自动加载）\n' + parts.join('\n\n')
}
...
export function apply(ctx) {
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    parameters: {},
    output: { schema: MEM_OBJ, render: (_a, v) => v.memory || v.error || '' },
    execute: (_args, exec) => {
    ...
    presentCall: () => 'Read project memory',
  })))
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    output: { schema: MEM_OBJ, render: (_a, v) => v.ok ? '已记录到项目 JOURNAL' : ('记录失败: ' + (v.error || '')) },
    execute: (args, exec) => {
      const cwd = cwdOf(exec.agent)
    ...
    presentCall: () => 'Log to project memory',
  })))
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    output: { schema: MEM_OBJ, render: (_a, v) => v.ok ? '已写入项目 FACT.md' : ('写入失败: ' + (v.error || '')) },
    execute: (args, exec) => {
      const cwd = cwdOf(exec.agent)
      if (!cwd) return Promise.resolve({ ok: false, error: '无法确定工作区' })
      try {
    ...
    presentCall: () => 'Write project fact',
  })))

  const offTurn = ctx.on('session/event', (session, event) => {
    const type = event?.type || ''
    if (!/turn\/end|turn\/complete|message\/complete|session\/end|session\/flush/.test(type)) return
    const cwd = session?.header?.cwd
    if (!cwd) return
    try {
      appendJournal(cwd, { ts: new Date().toISOString(), session: session?.id ?? null, tag: 'turn-end', auto: true, event: type })
    } catch { }
  })
